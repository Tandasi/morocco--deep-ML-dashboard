...
zone = st.sidebar.selectbox("Select Zone", ['zone1', 'zone2', 'zone3', 'zone4', 'zone5'])
...
df_scaled['Anomaly'] = iso_forest.fit_predict(df_scaled[features.columns.tolist() + ['Hour', 'Weekday']])
...
with col1:
    fig = px.line(df_scaled, x='DateTime', y=zone, color=df_scaled['Cluster'].astype(str),
                  title=f"{zone} Over Time by Cluster", labels={'color': 'Cluster'})
...
if pca_mode == "2D":
    ...
    fig_3d = px.scatter_3d(
        df_scaled, x='PCA1', y='PCA2', z='PCA3',
        color=df_scaled['Anomaly'].map({1: 'Normal', -1: 'Anomaly'}),
        symbol=df_scaled['Cluster'].astype(str),
